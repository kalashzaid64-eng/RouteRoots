...
class RideController extends Controller
{
    public function index(Request $request)
    {
        ...
        $rides = $rides->map(function ($ride) use ($userId) {
            $participant = $ride->participants()->where('user_id', $userId)->first();

            $ride->join_status = $participant ? $participant->pivot->status : null;
            $ride->is_joined = $ride->join_status === 'accepted';
    ...
    public function acceptRequest($rideId, $userId)
    {
        ...
        foreach ($joinedUser->followers as $follower) {
            NotificationService::send($follower->id, 'friend_joined_ride', [
                'friend_id' => $joinedUser->id,
                'friend_name' => $joinedUser->name,
                'ride_id' => $ride->id,
